...
(define-struct bird-str (x y v-y) #:transparent)
(define-struct pipe-str (gap-y x) #:transparent)
(define-struct variables-str (gravity momentum scroll-speed score) #:transparent)
(define-struct state-str (bird pipes variables) #:transparent)

(define bird1 (bird-str bird-x bird-initial-y 0))
...
(define (get-initial-state)
  (state-str bird1 (list (pipe-str (+ added-number (random random-threshold)) scene-width)) (variables-str initial-gravity initial-momentum initial-scroll-speed 0)))
...
(define (next-state-bird bird gravity)
  (match-let [((bird-str x y v-y) bird)]
    (struct-copy bird-str bird [y (+ y v-y)] [v-y (+ v-y gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let [((bird-str x y v-y) bird)]
    (struct-copy bird-str bird [v-y (- 0 momentum)])))
...
(define (change current-state pressed-key)
  (match-let [
              (bird (get-bird current-state))
              (momentum (variables-str-momentum (state-str-variables current-state)))
              ]
    (if (key=? pressed-key " ")
        (struct-copy state-str current-state [bird (next-state-bird-onspace bird momentum)])
        current-state)))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (current-pipe)
         (struct-copy pipe-str current-pipe [x (- (get-pipe-x current-pipe) scroll-speed)])) pipes))
...
(define (clean-pipes pipes)
  (filter (λ (elem)
            (> (pipe-str-x elem) (- 0 pipe-width))) pipes))
...
(define (add-more-pipes pipes)
  (let* [
         (first-pipe (car pipes))
         (first-pipe-x (get-pipe-x first-pipe))
         ]
    (if (< (length pipes) no-pipes)
        (cons (struct-copy pipe-str first-pipe [x (+ first-pipe-x pipe-width pipe-gap)] [gap-y (+ added-number (random random-threshold))]) pipes)
        pipes)))
...
(define (get-score state)
  (variables-str-score (state-str-variables state)))
...
(define (check-pipe-collisions bird pipes)
  (if (equal? pipes '())
      ...
      (let [(current-pipe (car pipes))]
        (if (or (check-collision-rectangles (make-posn (bird-str-x bird) (get-bird-y bird))
                                            (make-posn (+ (bird-str-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                                            (make-posn (get-pipe-x current-pipe) 0)
                                            (make-posn (+ (get-pipe-x current-pipe) pipe-width) (pipe-str-gap-y current-pipe)))
                (check-collision-rectangles (make-posn (bird-str-x bird) (get-bird-y bird))
                                            (make-posn (+ (bird-str-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                                            (make-posn (get-pipe-x current-pipe) (+ (pipe-str-gap-y current-pipe) pipe-self-gap))
                                            (make-posn (+ (get-pipe-x current-pipe) pipe-width) ground-y)))
            #t
...
(define (next-state state)
  (let [
        (next-bird (next-state-bird (get-bird state) (variables-str-gravity (state-str-variables state))))
        (next-pipes (next-state-pipes (get-pipes state) (variables-str-scroll-speed (state-str-variables state))))
        (next-score (+ 0.1 (get-score state)))
        ]
    (struct-copy state-str state [bird next-bird] [pipes next-pipes]
                 [variables (struct-copy variables-str (state-str-variables state) [score next-score])])))
...
(define (draw-frame state)
  (match-let [
              ((bird-str x y v-y) (get-bird state))
              ]
    (place-image bird-image (+ (quotient bird-width 2) x) (+ (quotient bird-height 2) y) 
                 (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2))
                              (place-image (score-to-image (get-score state)) text-x text-y
                                           (place-gaps (state-str-pipes state)
                                                       (place-pipes (state-str-pipes state) initial-scene)))))))
...
(define place-gap
  (λ (pipe scene)
    (place-image (rectangle pipe-width pipe-self-gap "solid" "white")
                 (+ (get-pipe-x pipe) (quotient pipe-width 2))
                 (+ (pipe-str-gap-y pipe) (quotient pipe-self-gap 2))
                 scene)))

(define place-gaps
  (λ (pipes scene)
    (foldl place-gap scene pipes)))
